...
if shader_enabled shader_set(shd_posterization);
    shader_set_uniform_f(uni_time, var_time_var);
    shader_set_uniform_f(uni_mouse_pos, var_mouse_pos_x, var_mouse_pos_y);
    shader_set_uniform_f(uni_resolution, var_resolution_x, var_resolution_y);
    shader_set_uniform_f(uni_gamma, var_gamma);
    shader_set_uniform_f(uni_num_colors, var_num_colors);
    if full_screen_effect draw_surface(surf,0,0);
shader_reset();

draw_text(0,0,string_hash_to_newline("Real FPS: "+string(fps_real)));
draw_text(0,12,string_hash_to_newline("Gamma amount (Q & A to adjust): "+string(var_gamma)));
draw_text(0,24,string_hash_to_newline("Number of colors (W & S to adjust): "+string(var_num_colors)));
