...
function add_to_inventory(object,num){
	var key=object_get_name(object.object_index);
...
function remove_from_inventory(object,num){
	var key=object_get_name(object.object_index);
	if(ds_map_exists(inventory,key)){
		if(ds_map_find_value(inventory,key)>=num){
			ds_map_set(inventory,key,ds_map_find_value(inventory,key)-num);
